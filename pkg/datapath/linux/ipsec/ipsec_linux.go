...
func ipSecJoinState(state *netlink.XfrmState, keys *ipSecKey, dir IPSecDir) {
...
func ipSecReplaceState(remoteIP, localIP net.IP, dir IPSecDir, mark uint32) (uint8, error) {
	state := ipSecNewState()
...
func UpsertIPsecEndpoint(local, remote *net.IPNet, dir IPSecDir) (uint8, error) {
	...
	if !local.IP.Equal(remote.IP) {
		if dir == IPSecDirIn || dir == IPSecDirBoth {
			if spi, err = ipSecReplaceState(local.IP, remote.IP, dir, linux_defaults.RouteMarkDecrypt); err != nil {
				if !os.IsExist(err) {
		...
		if dir == IPSecDirOut || dir == IPSecDirBoth {
			if spi, err = ipSecReplaceState(remote.IP, local.IP, dir, linux_defaults.RouteMarkEncrypt); err != nil {
				if !os.IsExist(err) {
...
func loadIPSecKeys(r io.Reader) (uint8, error) {
	...
	for scanner.Scan() {
		...
		if err != nil {
			// If no version info is provided assume using keu format without
