	...
	*state = snat_v4_lookup(tuple);
	if (*state)
		return 0;
		 ...
		 tuple->saddr == SNAT_IPV4_EXTERNAL)
		return 1;
	else
...
static __always_inline int snat_v4_rewrite_egress(struct __sk_buff *skb,
						  ...
						  __u32 off)
{
	__be32 sum_l4 = 0, sum = csum_diff(&tuple->saddr, 4,
					   &state->to_saddr, 4, 0);
...
static __always_inline int snat_v4_rewrite_ingress(struct __sk_buff *skb,
						   ...
						   __u32 off)
{
	__be32 sum_l4 = 0, sum = csum_diff(&tuple->daddr, 4,
					   &state->to_daddr, 4, 0);
	struct csum_offset csum = {};
	int ret;
...
static __always_inline int snat_v4_process(struct __sk_buff *skb, int dir)
{
	...
	ret = snat_v4_handle_mapping(skb, &tuple, &state, &tmp, dir);
	if (ret > 0)
		return TC_ACT_OK;
	if (ret < 0)
		return DROP_NAT_NO_MAPPING;
...
static __always_inline int snat_v6_rewrite_egress(struct __sk_buff *skb,
						  ...
						  __u32 off)
{
	__be32 sum = csum_diff(&tuple->saddr, 16, &state->to_saddr, 16, 0);
	struct csum_offset csum = {};
	int ret;
...
static __always_inline int snat_v6_rewrite_ingress(struct __sk_buff *skb,
						   ...
						   __u32 off)
{
	__be32 sum = csum_diff(&tuple->daddr, 16, &state->to_daddr, 16, 0);
	struct csum_offset csum = {};
	int ret;
...
static __always_inline int snat_v6_process(struct __sk_buff *skb, int dir)
{
	...
	ret = snat_v6_handle_mapping(skb, &tuple, &state, &tmp, dir);
	if (ret > 0)
		return TC_ACT_OK;
	if (ret < 0)
		return DROP_NAT_NO_MAPPING;
...
static __always_inline int snat_process(struct __sk_buff *skb, int dir)
{
	return
	       ...
	       skb->protocol == bpf_htons(ETH_P_IP)   ? snat_v4_process(skb, dir) :
